...
class AppStateManager with ChangeNotifier {
  ...
  Future<void> _syncCachedUserSession(Userdata cachedUser) async {
    ...
    } on MobileSessionExpiredException catch (exception) {
      print("saved session expired: $exception");
      await unsetUserData();
    } catch (exception) {
      print("saved session sync skipped: $exception");
    }
  }

  Future<void> _replaceUserData(Userdata _userdata) async {
    ...
    if (userdata != null && userdata!.activated == 0) {
      eventBus.fire(UserLoggedInEvent(userdata));
      updateUserToken();
      MoreMenuPreloadService.instance.warmQuietly(user: userdata, force: true);
    }
    notifyListeners();
  }
