    ...
    return await openDatabase(
      ...
      if (oldVersion < 4) {
        await _addColumnIfMissing(
            db, Userdata.TABLE, 'canManageGroups', 'INTEGER');
      }
      if (oldVersion < 5) {
        await _addColumnIfMissing(
            db, Userdata.TABLE, 'countryOfResidence', 'TEXT');
      }
      if (oldVersion < 6) {
        await _addColumnIfMissing(
      ...
      if (oldVersion < 7) {
        ...
        await _addColumnIfMissing(
            db, Userdata.TABLE, 'addressLatitude', 'TEXT');
        await _addColumnIfMissing(
            db, Userdata.TABLE, 'addressLongitude', 'TEXT');
      }
      if (oldVersion < 8) {
        await _addColumnIfMissing(
